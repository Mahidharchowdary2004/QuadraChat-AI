...
const ChatInterface = ({ tileId, onBack }: ChatInterfaceProps) => {
  ...
  useEffect(() => {
    ...
    if (!supabase) {
      ...
      toast({
        title: "Error",
        description: "Failed to initialize Supabase. Please check your configuration.",
        variant: "destructive",
      });
      return;
    }
  ...
  const retryLoadMessages = () => {
    setError(null);
    loadMessages(tileId, sessionId).then((result: LoadMessagesResult) => {
      if (result.error) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const { id, error: saveError } = await saveMessage(tileId, sessionId, "user", userMessage);
      ...
      const { data: history, error: loadError } = result as LoadMessagesResult;
      ...
      const { id: aiId, error: aiSaveError } = await saveMessage(tileId, sessionId, "assistant", aiResponse);
